...
enum symbol {
    ...
    varsym,      procsym,   floatsym,//extend
    intarrsym,//extend
    floatarrsym,//extend
    floorsym,//extend
    ceilsym,//extend
...
struct instruction
{
    ...
    int l;      /* 引用层与声明层的层次差 */
    int a;      /* 根据f的不同而不同 */
    float isFlow;//extend 涉及浮点变量时使用 0为存整数 1为存浮点
};
...
#define intarrdeclarationdo(a,b,c);    if(-1 == intarrdeclaration(a,b,c)) return -1//extend 
#define floatarrdeclarationdo(a,b,c);    if(-1 == floatarrdeclaration(a,b,c)) return -1//extend 
